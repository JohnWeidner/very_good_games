...
/// Results overlay displayed when the Chromix puzzle is solved.
class ChromixResultsOverlay extends StatelessWidget {
  /// Creates a [ChromixResultsOverlay].
  const ChromixResultsOverlay({
    required this.state,
    this.onViewPuzzle,
    super.key,
  });
  ...
  /// Called when the user wants to dismiss the overlay to view
  /// the puzzle.
  ...
  Widget build(BuildContext context) {
    ...
    final stars = chromixStars(score, state.optimalMoves);
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    LeaderboardSection(dTag: 'chromix:${utcDateKey()}'),
                    const SizedBox(height: 24),
                    Row(
  ...
  void _share(BuildContext context) {
    ...
    context.read<ResultSharingCubit>().share(
      eventBuilder: ({required pubKeyHex, required date}) =>
          EventBuilder.buildChromixResult(
            pubKeyHex: pubKeyHex,
            score: score,
            stars: stars,
